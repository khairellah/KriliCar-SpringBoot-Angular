...
import { CompanyRegistrationRequest } from '../../../core/models/auth/company-registration-request.model';
...
// Même validateur de groupe que RegisterClientComponent : confirmation 100% Frontend,
...
export class RegisterCompanyComponent {
  ...
  readonly form: FormGroup<RegisterCompanyForm> = this.fb.nonNullable.group(
    {
  ...
  onSubmit(): void {
    ...
    const { confirmPassword, city, landline, description, ...rest } = this.form.getRawValue();

    // ⚠️ SÉCURITÉ (US-1.3) : aucun champ isBooster n'est construit ni envoyé ici.
    const payload: CompanyRegistrationRequest = {
      ...rest,
      city: city as City,
    ...
    this.authService.registerCompany(payload, this.selectedImage()).subscribe({
      next: () => {
        this.isLoading.set(false);
        this.successMessage.set(
          'Votre société a été inscrite avec succès. Vous pouvez maintenant vous connecter.'
        );
        this.form.reset();
        this.selectedImage.set(null);
        this.imagePreviewUrl.set(null);
